...
const ArticleList = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await fetch("http://localhost/wp-first//wp-json/wp/v2/posts?_embed=1");
        if (!response.ok) {
  ...
  const handleAddArticle = async () => {
    try {
      const authString = "Ali:x6uc MZe3 VC7t ReOa fBQV t3AT";
      const encodedAuthString = btoa(authString);
      const apiUrl = "http://localhost/wp-first//wp-json/wp/v2/posts";
  ...
  const handleEditArticle = async () => {
    try {
      const authString = "Ali:x6uc MZe3 VC7t ReOa fBQV t3AT";
      const encodedAuthString = btoa(authString);
      const apiUrl = `http://localhost/wp-first//wp-json/wp/v2/posts/${selectedArticle.id}`;
      const response = await fetch(apiUrl, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
